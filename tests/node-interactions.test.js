...
assert.match(interactions, /let lastTouchNodeClick = \{ node: null, time: 0 \};/);
...
assert.match(interactions, /!touchInteraction &&\s*lastTouchNodeClick\.node === node &&\s*Date\.now\(\) - lastTouchNodeClick\.time < 800/);
assert.match(interactions, /if \(touchInteraction\) \{\s*lastTouchNodeClick = \{ node, time: Date\.now\(\) \};\s*if \(selectedNode === node\) \{\s*showNodePanel\(node\);\s*\} else \{\s*setNodePanelOpen\(false\);\s*selectedNode = node;/);
