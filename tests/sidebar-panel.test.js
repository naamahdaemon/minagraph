...
assert.match(css, /\.layout-toggle-button\s*\{[\s\S]*?-webkit-touch-callout:\s*none;[\s\S]*?user-select:\s*none;[\s\S]*?touch-action:\s*manipulation;/);
...
assert.match(script, /button\.setPointerCapture\?\.\(event\.pointerId\)/);
