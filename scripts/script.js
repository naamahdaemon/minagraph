...
function bindStablePinchReference(container) {
  ...
  container.addEventListener("pointerdown", event => {
    ...
    if (isFullscreen && fullscreenUiVisible) {
      // Once the temporary fullscreen interface is visible, any new touch on
      // the graph hides it immediately, including a node drag or pinch. Do not
...
function setupInteractions() {
  ...
  renderer.on("clickStage", ({ event }) => {
    ...
    if (
      ...
      Date.now() > suppressFullscreenUiRevealUntil &&
