...
let fullscreenUiSidebarRequested = false;
let sidebar;
let details;
let tooltip;
let panel;
...
function bindStablePinchReference(container) {
  ...
  container.addEventListener("pointerdown", event => {
    if (isFullscreen && fullscreenUiVisible) {
      // Once the temporary fullscreen interface is visible, any new pointer
      // interaction on the graph hides it immediately. Do not
      // let the click generated by this same gesture reveal it again.
      suppressFullscreenUiRevealUntil = Date.now() + 800;
...
function setupInteractions() {
  ...
  renderer.on("clickStage", ({ event }) => {
    ...
    const touchStageClick = isTouchInteraction(event);
    const qualifiedStageReveal = !touchStageClick || Date.now() - lastQualifiedGraphTouchTapAt < 300;
    if (isFullscreen && Date.now() > suppressFullscreenUiRevealUntil && qualifiedStageReveal) {
      lastQualifiedGraphTouchTapAt = 0;
      setFullscreenUiVisible(true, { openSidebar: false });
      return;
    }
...
function setFullscreenUiVisible(visible, { openSidebar = true } = {}) {
...
document.addEventListener("keydown", function (event) {
  ...
  if (event.key === "s" || event.key === "S") {
    if (isFullscreen) {
      event.preventDefault();
      setFullscreenUiVisible(!fullscreenUiVisible, { openSidebar: true });
      return;
    }
